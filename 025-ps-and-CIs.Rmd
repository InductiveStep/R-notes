...
S-values are a neat idea for helping to think about -- maybe even (in some sense) *feel* -- the meaning of p-values! They're described by [Rafi and Greenland (2020)](https://doi.org/10.1186/s12874-020-01105-9). Let me try to illustrate the gist, starting with flips of a coin.
...
* Flip the coin 20 times. All of the outcomes are heads. How surprised now?

I suspect your level of surprise has gone from something like "meh" through to "WOW WHAT?!"

S-values are a way to think about p-values in terms of how surprised you would be by a sequence of heads on coin flips, the number calculated from the $p$ you got, if the coin were fair.
...
Now here is the connection with p-values. Suppose you get $p = 0.03125$; that's the same probability as the probability of obtaining five heads in a row from five coin tosses. The s-value is 5.
...
So that's the same as getting all heads when you flip a coin 4.32 times -- which isn't entirely intuitive when expressed as coin flips. But you could think of it as surprising as at least 4 but not 5 heads in a row.

Here are examples for common thresholds:
