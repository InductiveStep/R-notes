...
The NatCen report opens with a spotlight figure:
...
I've also filtered the data so that participants are aged 18 or over.
...
Check it works for GB:

```{r}
gimme_country_results("GB")
```

Run it for all the countries of interest:

```{r}
conts <- c("DE", "ES", "FR", "GB", "IT")
euro_wealth <- map_dfr(conts, gimme_country_results)
head(euro_wealth)
```

Looks like it worked. We can zoom in on "Fair" and calculate a rounded percentage to compare with Table 1 in the report:

```{r}
euro_wealth %>%
  filter(wltdffr_group == "Fair") %>%
  mutate(perc = round(prop*100, 0)) %>%
  select(cntry, wltdffr_group, perc)
```
